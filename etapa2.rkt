...
(define (get-suffixes text)
  (if (null? text)
      `()
      (append (cons text `()) (get-suffixes (cdr text)))))
...
(define (ast-func suffixes)
  (if (or (equal? (car suffixes) `(#\$)) (equal? suffixes `(#\$)))
      `((#\$))
      (foldl (λ (x acc)
               (append acc (cons (cadr (longest-common-prefix x (car acc))) `()))) (cons (cons (caar suffixes) `()) `()) suffixes)))
...
(define (cst-func suffixes)
  (if (or (equal? (car suffixes) `(#\$)) (equal? suffixes `(#\$)))
      `((#\$))
      (foldl (λ (x acc)
               (append acc (cons (cadr (longest-common-prefix x (car acc))) `()))) (cons (longest-common-prefix-of-list suffixes) `()) suffixes)))
...
(define (check-nulls L)
  (cond
    [(null? L) `()]
    [(null? (car L)) (check-nulls (cdr L))]
    [else (append (list (car L)) (check-nulls (cdr L)))]))
...
(define (text->st func)
  (λ (L)
    (suffixes->st func (get-suffixes (append L `(#\$)))  (sort (append `(#\$) (remove-duplicates L)) char<?))))

;(append (sort (remove-duplicates (string->list "banana")) char<?) `(#\$))
;(suffixes->st ast-func (get-suffixes (append (string->list "banana") `(#\$))) (append `(#\$) (sort (remove-duplicates (string->list "banana")) char<?)))
;(string->list "abn$")
